...
export default function StudentDashboardScreenig() {
  ...
  return (
    <div style={{ background: backgroundColor, minHeight: "100vh" }}>
      <div
        className="grid grid-cols-1 sm:grid-cols-3 gap-7 px-10 py-20 relative overflow-auto max-w-full h-auto"
        ...
        <div className="flex flex-col space-y-4">
          <div className="card bg-white shadow-lg rounded-lg p-6">
            <div className="text-2xl font-bold" style={{ color: textColor }}>
              Number of Interviews Allocated
            </div>
            <div className="text-xl" style={{ color: textColor }}>
              {userStats?.cards?.length > 0 ? userStats?.cards[0]?.value : ""}
            </div>
          </div>
          <div className="card bg-white shadow-lg rounded-lg p-6">
            <div className="text-2xl font-bold" style={{ color: textColor }}>
            ...
            <div className="text-xl" style={{ color: textColor }}>
              {userStats?.cards?.length > 0 ? userStats?.cards[1]?.value : ""}
            </div>
          </div>
        </div>
      </div>
