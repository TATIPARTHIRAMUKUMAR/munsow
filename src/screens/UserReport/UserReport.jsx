...
const UserReport = () => {
  ...
  const handleGeneratePdf = async () => {
    ...
    const pdfHeight = (pdfContainer.clientHeight * pdfWidth) / pdfContainer.clientWidth; // Maintain aspect ratio
      ...
      <div className="container mx-auto">
        ...
        <div ref={reportTemplateRef} className="bg-white">
          ...
          {reportData?.report_type == "skill based report" && (
            <div>
              ...
              <SkillSuggestions
                data={
                  reportData?.skill_based_suggestions ? reportData?.skill_based_suggestions : {}
                }
              />
              <Divider className="pt-5" />
            </div>
          ...
          {reportData?.report_type == "skill based report" && (
            <div>
              ...
              <SkillsDisplay
                skills={
                  reportData?.hard_and_soft_skill_dic ? reportData?.hard_and_soft_skill_dic : {}
                }
              />
              <Divider className="pt-5" />
            </div>
        ...
        <div className="mt-5">
          <div className="flex justify-center items-center mt-4 mb-5">
            ...
            >
              DOWNLOAD AS PDF{" "}
              {loading && <CircularProgress style={{ color: "#fff", marginLeft: "10px" }} />}
            </button>
          </div>
        </div>
      </div>
