        ...
        <div className="flex flex-col h-full p-4 bg-white">
          <div className="flex-grow p-2 flex flex-col gap-y-2">
            {skill_type == "role based report" ? (
              ...
              <div className="text-xl ">
                <span className="font-semibold"> Skill based report</span>
                <div className="text-base ">
                  <span className="font-semibold"> Skills : </span>
                  {Object.keys(skills_list?.hard_skill || {}).map(
                    (skill, index) => (
                      <>
                  ...
                  {Object.keys(skills_list?.soft_skill || {}).map(
                    (skill, index) => (
                      <>{skill}</>
                    )
                  )}
            ...
            <div className="font-medium">
              ...
              {moment(generated)?.format("MMMM DD, YYYY HH:mm:ss")}
            </div>
          </div>
          ...
          <div className="py-2 w-full">
            {report_ready === "true" ? (
              <>
                <Button
                  className="font-bold w-full py-2 cursor-pointer transition-colors duration-300 hover:bg-[#886cc0] "
            ...
            >
              {lessonsList?.map((o, index) => (
                <ReportCards
                  ...
                  report_data={o?.report_json == null ? {} : o?.report_json}
                  report_ready={o?.report_json == null ? "false" : "true"}
                  skills_list={o?.report_json?.hard_and_soft_skill_dic}
                  level={o?.level}
                  generated={o?.updated_date}
                  key={index}
                  company={o?.report_json?.interview_company}
                />
              ))}
            </div>
          )}
