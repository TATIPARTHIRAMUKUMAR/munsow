...
export default function ReportIndex() {
  ...
  const handleFilterChange = event => {
    ...
    const filteredReports = userReportList.filter(report =>
      report.report_type.includes(event.target.value)
    );
    ...
    const renderSkillsSection = (skills, title) => {
      ...
      return (
        <>
          ...
          {skills.map(skill => (
            <div key={skill} style={{ fontSize: "13px" }}>
              {skill}
            </div>
          ))}
        </>
      ...
      <div className="transition-transform duration-300 hover:scale-105 shadow-lg rounded-lg">
        <div className="flex flex-col h-full p-4 bg-white">
          <div className="flex-grow p-2 flex flex-col gap-y-2">
            {report_type === "company_role_interview" ? (
              ...
              <div className="text-xl font-semibold">
                Skill based report
                <div className="text-base text-[#886cc0]">Skills : {renderSkills(skills_list)}</div>
              </div>
            ...
            {(report_type === "company_role_interview" || report_type === "skill_interview") && (
              <div className="font-medium">
                <span className="font-bold">Level:</span> {level}
              </div>
            )}
            <div className="font-medium">
              <span className="font-bold">Generated On:</span> {formattedDate} IST
            </div>
          </div>
          ...
          <div className="py-2 w-full">
            {report_ready === "true" ? (
              <Button
                className="font-bold w-full py-2 transition-colors duration-300"
                endIcon={<ArrowForwardIcon />}
                onClick={() => viewReport({ ...report_data, ...result_data })}
  ...
  return (
    <>
      <div className="p-7 flex justify-between ">
        <Pagination
          count={Math.ceil(lessonsList.length / reportsPerPage)}
          page={currentPage}
            ...
            >
              {currentReports.map((o, index) => (
                <ReportCards
                  key={index}
                  id={o.id}
                  report_type={o.interview_type}
                  role={o.specifications?.role}
                  skill_type={o.report_json?.report_type}
                  report_data={o.report_json || {}}
                  result_data={o.result_json || {}}
                  report_ready={o.report_json ? "true" : "false"}
                  skills_list={o.report_json?.hard_and_soft_skill_dic}
                  level={o.level}
