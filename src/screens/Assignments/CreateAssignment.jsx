...
const CreateAssignment = () => {
  ...
  const handleNext = () => {
    if (currentStep === 0) {
      ...
      dispatch(
        create_assignment(dataToSubmit, resp => {
      ...
      dispatch(
        assign_assignment(payload, resp => {
          setCurrentStep(prevStep => prevStep + 1);
        })
      );
                ...
                >
                  {questionBanksList.map(option => (
                    <MenuItem key={option.id} value={option}>
                      <Checkbox
                        checked={assignmentDetails.question_bank_ids.indexOf(option) > -1}
                      />
