  ...
  return (
    <div className="relative container mx-auto p-20 m-30">
      <svg
        ...
        className="w-full h-12 absolute top-1/2 transform -translate-y-1/2 left-0 right-0"
      >
        <path
          d="M0,5 C20,-5 40,15 60,5 S80,-5 100,5"
          fill="transparent"
          stroke="#374151"
          stroke-width="2"
        ></path>
        ...
        {detailedCourse?.content_data?.map((topic, index) => (
          <div key={topic.id} className="flex flex-col items-center relative">
            <button
              ...
              className={`mt-4 rounded-full w-12 h-12 flex items-center justify-center shadow-lg text-white ${
                selectedTopic === topic ? "bg-blue-600" : "bg-blue-500 hover:bg-blue-600"
              }`}
            >
              <PlaceIcon />
            </button>
