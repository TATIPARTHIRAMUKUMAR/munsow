      ...
      if (editMode) {
        ...
        setTopics(prevTopics =>
          prevTopics.map(topic =>
            topic.id === editTopicId ? { ...topic, name: newTopicTitle } : topic
          )
        );
        setEditMode(false);
      } else {
    ...
    <div className="bg-gray-200 w-full h-screen p-5">
      <div className="relative w-full bg-white rounded-xl">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-4">
          <div className="inline-flex items-center space-x-2 ">
            <span className="text-2xl text-black font-semibold">Topic & Subtopic Creation</span>
          </div>
      ...
      <div className="relative w-full mt-5">
        {topics.map((topic, index) => (
          <div key={index}>
            <div
              className={`mx-auto flex max-w-7xl items-center justify-between px-4 py-2 sm:px-6 lg:px-8 bg-white rounded-full mt-2 p-4 ${highlightedIndex === index ? "bg-[rgb(0,0,0,0.12)]" : ""}`}
            >
              <div className="inline-flex items-center space-x-2">
                <button>
                  <img src={equals} alt="" className="w-5 h-5" />
                </button>
                <div className="w-auto pl-5">{topic.name}</div>
              </div>
              <div className="flex justify-between gap-7">
                ...
                <button
                  onClick={() =>
                    addSubtopic(index, `${topic.name} ${index + 1}.${topic.subtopics.length + 1}`)
                  }
                >
            ...
            <div className="relative w-full pl-20 mb-5">
              {topic.subtopics?.map((subtopic, subIndex) => (
                <div
                  className="mx-auto flex max-w-7xl items-center justify-between px-2 py-2 sm:px-6 lg:px-8 bg-white  rounded-full mt-2 p-4"
                  key={subIndex}
                >
                  <div className="inline-flex items-center space-x-2">
                    <button className="">
                    ...
                    <div className="ml-5">{subtopic?.name}</div>
                  </div>
