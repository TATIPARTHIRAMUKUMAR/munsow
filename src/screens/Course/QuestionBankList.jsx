...
const QuestionBankCard = ({ course, onClick }) => {
...
const QuestionBankList = () => {
    ...
    const { questionBanks } = useSelector((state) => state?.data);
    ...
    return (
        <div className="max-w-7xl mx-auto p-8">
            <div className="mb-6 flex justify-between items-center">
                <h1 className="text-3xl font-bold">Question Banks List</h1>
                <button
                    onClick={() => navigate("/questionBankList/createQuestionBank")}
                    className="bg-[#886CC0] text-white px-4 py-2 rounded-md transition duration-300 hover:bg-blue-600 focus:outline-none focus:ring focus:ring-blue-300"
                >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
                {questionBanks?.map((course) => (
                    <QuestionBankCard key={course?.id} course={course} onClick={handleCardClick} />
                ))}
            </div>
        </div>
    );
