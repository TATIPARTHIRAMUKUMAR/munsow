  ...
  return cardLists?.map((cardItem, index) => {
    ...
    return (
      <div
        style={{
          background:
            "linear-gradient(180.43deg,  #25c4b4 19.43%, #2BE2D0 87.63%)",
        }}
        className=" mb-3 p-4 rounded-lg shadow-md grid items-"
        key={index}
      >
        <div className="w-6 h-6 mb-3">
          {/* {icon} */}
          {React.cloneElement(icon, { style: { fill: "#000000 !important" } })}
        </div>
        <div
          className="text-truncate text-[#252525] text-lg mt-6 font-bold"
          title={cardContent}
        >
          {cardContent}
        </div>
