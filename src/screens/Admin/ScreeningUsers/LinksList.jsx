...
const LinksList = () => {
  ...
  const handleToggleChange = link => {
    dispatch(
      updateLinkStatus(link.unique_code, link.is_active ? "deactive" : "active", () => {
        dispatch(loadLinks());
      })
    );
  };
      ...
      <TableContainer component={Paper}>
        <Table sx={{ minWidth: 650 }} aria-label="customized table" id="links-list-table">
          <TableHead style={{ backgroundColor: "#F0F0F0" }}>
            <TableRow>
              <TableCell style={{ fontWeight: "bold", backgroundColor: "#F0F0F0", color: "black" }}>
                Name
              </TableCell>
              <TableCell style={{ fontWeight: "bold", backgroundColor: "#F0F0F0", color: "black" }}>
                Description
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center p-4 border-b border-gray-200">
          <div>
            <h2 className="text-lg font-semibold">
              Link : <span style={{ color: "#2BE2D0" }}>{selectedItem?.name}</span> Users
            </h2>
          </div>
        ...
        <div className="p-4">
          <TableContainer component={Paper}>
            <Table sx={{ minWidth: 650 }} aria-label="customized table">
              ...
              <TableBody>
                {screeningUsers?.data
                  ?.slice(page * rowsPerPage1, page1 * rowsPerPage1 + rowsPerPage1)
                  .map(row => (
                    <TableRow key={row.id}>
