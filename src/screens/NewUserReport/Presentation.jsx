    ...
    <>
      <div className="mx-3 my-3 md:mx-6 md:my-6">
        <div className="flex justify-around items-center pb-4 lg:pb-8 munsow-dark-bg">
          <h1 className="mx-4 max-[375px]:text-xl text-2xl lg:text-4xl font-semibold text-white">
            Presentation and Grooming
          </h1>
          <div className="mx-4 text-center bg-white p-2 lg:p-6 rounded-b-3xl">
            <h1
              className={`max-[375px]:text-2xl text-3xl lg:text-4xl font-bold ${getScoreColor(formatScore(roundedScore))}`}
            >
              {roundedScore ? `${formatScore(roundedScore)}/10` : "0/10"}
            </h1>
        ...
        <div className="p-4 grid max-w-xl grid-cols-1 gap-x-8 gap-y-6 lg:max-w-none lg:grid-cols-3 lg:gap-y-16">
          {behavioral_presentation_and_grooming?.data?.map((value, index) => (
            <div className="">
              <h1
                className={`text-center text-4xl font-bold ${getScoreColor(formatScore(value.secured_marks))}`}
              >
                {value.secured_marks ? `${formatScore(value.secured_marks)}/10` : "0/10"}
              </h1>
