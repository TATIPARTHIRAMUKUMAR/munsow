...
const getStarRating = value => {
  ...
  for (let i = 0; i < remainingStars; i++) {
    stars.push(
      <span key={fullStars + i} className="text-yellow-500">
        &#9734;
      </span>
    ); // Empty star
  }
...
const SkillsDisplay = ({ skills }) => {
  ...
  return (
    <div
      ref={skillsRef}
      className="bg-[#212E3E] h-screen flex flex-col justify-center items-center m-5"
    >
      <h1 className="text-4xl text-white mb-8 font-semibold">
        Interview Skills in Decreasing Order of Performance
      </h1>
