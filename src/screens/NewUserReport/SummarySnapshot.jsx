...
const SummarySnapshot = props => {
  const {
    ...
    presentation_and_grooming_score,
    readiness_score,
    head,
    report_data,
  } = props;
      ...
      <div className="py-10 mx-3 my-3 md:mx-6 md:my-6 bg-grey">
        ...
        <div className="mx-4 my-8 md:mx-6 md:my-12 rounded-3xl py-6 bg-white flex justify-around ">
          <div className="text-center">
            <h1
              className={`text-2xl md:text-4xl font-bold ${getScoreColor(formatScore(readinessScore))} mb-4`}
            >
              {readinessScore ? `${formatScore(readinessScore)}/10` : "0/10"}
            </h1>
            <h3 className="max-[375px]:text-base text-xl font-bold text-purple">
              Overall Readiness Score
            </h3>
          </div>
          <div className="text-center">
            <h1
              className={`text-2xl md:text-4xl font-bold ${getScoreColor(formatScore(presentationScore))} mb-4`}
            >
              {presentationScore ? `${formatScore(presentationScore)}/10` : "0/10"}
            </h1>
            <h3 className="max-[375px]:text-base text-xl font-bold text-purple">
              ...
              {report_data?.graph_data?.map(jd => (
                <div className="px-4 py-2 block md:flex items-center">
                  ...
                  <div className="basis-1/4 text-center md:text-center">
                    <p className={`bold text-xl ${getScoreColor(jd?.scored)}`}>
                      {jd?.scored}/{jd?.total}
                    </p>
                  </div>
                </div>
            ...
            <div className="mx-4 md:mx-6 my-8 rounded-3xl py-6 bg-white">
              {report_data?.graph_data?.map(cultural => (
                <div className="px-4 py-2 block md:flex items-center">
                  ...
                  <div className="basis-1/4 text-center md:text-center">
                    <p className={`bold text-xl ${getScoreColor(cultural?.scored)}`}>
                      {cultural?.scored}/{cultural?.total}
                    </p>
                  </div>
                </div>
              ))}
