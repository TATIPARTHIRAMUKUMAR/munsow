...
export default function ReportIndex() {
  ...
  const handleFilterChange = event => {
    ...
    const filteredReports = userReportList.filter(report =>
      report.interview_type.includes(event.target.value)
    );
  ...
  const renderSkillCapsules = skillList => {
    if (skillList.length === 0) {
      ...
      return skillList.map((skill, index) => (
        <span
          key={index}
          className="bg-pink-100 px-1.5 py-1 rounded-xl break-words"
        ...
        <div className="p-3 mb-2">
          <div className="bg-zinc-200 flex" style={{ height: "12px", width: "100%" }}>
            <div
              ...
              style={{ height: "12px", width: `${(skillBasedCount / lessonsList?.length) * 100}%` }}
            ></div>
            <div
              className="bg-purple-500"
              style={{ height: "12px", width: `${(roleBasedCount / lessonsList?.length) * 100}%` }}
            ></div>
            <div
              className="bg-orange-500"
              style={{ height: "12px", width: `${(JDBasedCount / lessonsList?.length) * 100}%` }}
            ></div>
            <div
              className="bg-blue-500"
              style={{
                height: "12px",
                width: `${(CulturalFitCount / lessonsList?.length) * 100}%`,
              }}
            ></div>
          </div>
        </div>
  ...
  const ReportCards = ({
    ...
    const formattedDate = moment(generated)
      .add(5, "hours")
      .add(30, "minutes")
    ...
    return (
      <div
        className="transition-transform duration-300 hover:scale-105 shadow-lg bg-white"
        style={{ borderRadius: "18px", border: "2px solid #d9dde3" }}
      >
        <div className="flex flex-col h-full p-1.5" style={{ borderRadius: "18px" }}>
            ...
            {interview_type == "skill_interview" ? (
              <>
                <div className="flex gap-3 items-center">
                  <span
                    className={`${getDarkBgColor(interview_type)} rounded-md`}
            ...
            {interview_type == "company_role_interview" ? (
              <>
                ...
                <div className="font-medium flex flex-wrap items-center gap-2">
                  ...
                  <span
                    className="bg-purple-100 px-1.5 py-1 rounded-xl break-words"
                    style={{ maxWidth: "220px" }}
                  >
                    {role}
                  </span>
            ...
            {interview_type == "jd_interview" ? (
              <>
                ...
                <div className="font-medium flex flex-wrap items-center gap-2">
                  <span className="font-bold">Role:</span>
                  <span
                    className="bg-orange-100 px-1.5 py-1 rounded-xl break-words"
                    style={{ maxWidth: "220px" }}
                  >
                    {role}
                  </span>
            ...
            {interview_type == "cultural_interview" ? (
              <>
                ...
                <div className="font-medium flex flex-wrap items-center gap-2">
                  <span className="font-bold">Role:</span>
                  <span
                    className="bg-blue-100 px-1.5 py-1 rounded-xl break-words"
                    style={{ maxWidth: "220px" }}
                  >
                    {role}
                  </span>
          ...
          <div className="p-3 w-full text-right">
            {report_ready === "true" ? (
              <>
                <Button
                  className="font-bold py-3 cursor-pointer transition-colors duration-300 hover:bg-[#886cc0] "
                  endIcon={<ArrowForwardIosIcon style={{ marginLeft: "0px" }} />}
                  onClick={() => {
                    viewReport({ ...report_data, ...result_data });
                  }}
                  style={{
                    color: textColor,
  ...
  return (
    <>
      ...
      {lessonsList.length === 0 ? (
        ...
        <div
          className="w-full h-full overflow-y-auto px-1 md:px-4"
              ...
              >
                {filteredLessonsList.map((o, index) => (
                  <ReportCards
                    ...
                    role={o.specifications?.role}
                    skill_type={o.report_json?.interview_type}
                    report_data={o.report_json || {}}
                    result_data={o.result_json || {}}
                    report_ready={o.report_json ? "true" : "false"}
                    skills_list={o.specifications}
                    level={o.level}
                    generated={o.updated_date}
                    company={o.specifications?.company}
                    jdSkills={o.specifications?.jd_skill}
                    cultSkills={o.specifications?.cultural_skill}
                  />
                ))}
              </div>
            </>
