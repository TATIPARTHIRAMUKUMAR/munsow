            ...
            <div className="p-4 md:p-8">
              {Object.keys(skillSuggestions).map((skill, index) => (
                <div key={index} className="mb-4">
                  <h4 className="p-4 text-xl font-semibold text-purple mb-4 munsow-dark-bg text-white">
                    {skill}
                  </h4>
            ...
            <div className="p-4 md:p-8">
              ...
              {report_data?.industry_trends && report_data?.industry_trends.length > 0 && (
                <div className="mb-4">
                  ...
                  <ol className="pl-4 text-purple">
                    {report_data?.industry_trends.map(
                      (suggestion, Index) =>
                        suggestion && (
                          <li key={Index} className="mb-1 list-disc">
                            {suggestion}
              ...
              {report_data?.lastest_company_news &&
                report_data?.lastest_company_news.length > 0 && (
                  <div className="mb-4">
                    <h4 className="p-4 text-xl font-semibold text-purple mb-4 munsow-dark-bg text-white">
                      Latest Company News
                    </h4>
                    <ol className="pl-4 text-purple">
                      {report_data?.lastest_company_news.map(
                        (suggestion, Index) =>
                          suggestion && (
                            <li key={Index} className="mb-1 list-disc">
                              {suggestion}
              ...
              {report_data?.role_specific_skills &&
                report_data?.role_specific_skills.length > 0 && (
                  <div className="mb-4">
                    <h4 className="p-4 text-xl font-semibold text-purple mb-4 munsow-dark-bg text-white">
                      Role Specific Skills
                    </h4>
                    <ol className="pl-4 text-purple">
                      {report_data?.role_specific_skills?.map(
                        (suggestion, Index) =>
                          suggestion && (
                            <li key={Index} className="mb-1 list-disc">
                              {suggestion}
