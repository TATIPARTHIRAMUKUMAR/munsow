...
const QuizBrief = () => {
  ...
  return (
    <div className="quiz-container">
      <h1
        className="quiz-title font-bold mx-[150px]"
        style={{
          ...
          <div className="options">
            {currentQuestion?.options &&
              Object.entries(currentQuestion.options).map(([key, value]) => (
                <button
                  key={key}
                  className={`option ${answers[currentQuestionIndex] === key ? "selected" : ""}`}
        ...
        <>
          ...
          <div className="popup-container fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
            <div className="popup-content bg-white text-[#242D36] rounded-xl shadow-lg p-8 max-w-md w-full text-center animate-fade-in">
              <h2 className="text-3xl font-bold text-[#0fe1d2] mb-4">🎉 Congratulations! 🎉</h2>
              <p className="text-lg mb-2">
                Your Score:{" "}
                <strong className="text-[#0fe1d2]">
                  {score} / {questions.length}
                </strong>
              </p>
              <p className="text-sm mb-6">
