...
const FlashcardSelection = ({ selectedSubtopicName, flashcards }) => {
  ...
  const handleStart = () => {
    if (selectedLevel) {
      const selectedFlashcards = flashcards.filter(card => card.category === selectedLevel);
  ...
  const cards = [
    {
      title: "Knowledge",
      gradient: "linear-gradient(to right, #0CC7C2, #1F5A5E)",
      cardCount: `${categoryCounts["Knowledge"] || 0} cards`,
      border: "#1F5A5E",
    },
    {
      title: "Understanding",
      gradient: "linear-gradient(to left, #30638E, #242D36)",
      cardCount: `${categoryCounts["Understanding"] || 0} cards`,
      border: "#30638E",
    },
    {
      title: "Examples and Applications",
      gradient: "linear-gradient(to right, #08ADB0, #186E8D)",
      cardCount: `${categoryCounts["Examples and Applications"] || 0} cards`,
      border: "#186E8D",
    },
    {
      title: "Case Studies",
      gradient: "linear-gradient(to left, #0FE1D3, #003D5B)",
      cardCount: `${categoryCounts["Case Studies"] || 0} cards`,
      border: "#0FE1D3",
    },
  ];
          ...
          >
            <div className="flashcard-details">
              <div className="card-info">
                <div className="card-count float-left" style={{ borderColor: card.border }}>
                  {card.cardCount}
                </div>
        ...
        <Box sx={style}>
          {filteredFlashcards && filteredFlashcards?.length > 0 ? (
            <>
              ...
              <div className="gallery-container mb-5">
                <div className="gallery-track">
                  <div className={`slide-card ${isFlipped ? "flipped" : ""}`} onClick={handleFlip}>
            ...
            <div className="flex flex-col items-center">
              ...
              <h2 className="text-center text-2xl font-bold text-red mt-5">
                "No flashcards found for the specified course."
              </h2>
            </div>
          )}
        </Box>
