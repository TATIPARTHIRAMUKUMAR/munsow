...
export default function Audio_Video({
  ...
  const stopRecording = () => {
    ...
    mediaRecorder.current.onstop = () => {
      ...
      alert(
        "Your voice recording has been captured. In a real-world scenario, we would now validate it against the provided text."
      );
    };
  };
  ...
  return (
    <div className="grid gap-4 justify-center p-2">
      ...
      <div className="w-full flex justify-center gap-4">
        {permission ? (
          <VideocamIcon
            className="text-purple-500 bg-white p-1 rounded-full cursor-pointer"
            fontSize="large"
            onClick={capOff}
            style={{ color: linearGradientBackground }}
          />
        ) : (
          <VideocamOffIcon
            className="text-purple-500 bg-white p-1 rounded-full cursor-pointer"
      ...
      <div className="text-center mt-4 max-w-screen-md mx-auto overflow-auto">
        ...
        <blockquote className="text-gray-700 italic text-sm sm:text-md lg:text-base">
          {sampleText}
        </blockquote>
      </div>
